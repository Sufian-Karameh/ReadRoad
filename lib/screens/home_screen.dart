  ...
  Widget getAllComments(String postId, List docList) {
    ...
    return ListView.builder(
      ...
      itemBuilder: (BuildContext context, int index) {
        return //Text("data");
            getCommentWidget(docList[index]["username"], docList[index]["text"],
                docList[index]["icon"]);
      },
    );
  }
    ...
    return Row(
      ...
      children: [
        ...
        Padding(
          ...
          child: CircleAvatar(
            backgroundImage: AssetImage("lib/Icons/$iconNum.png"),
            radius: 25,
          ),
        ),
        Expanded(
        ...
        children: [
          ...
          Row(
            children: [
              if (isUserSignedIn())
                CircleAvatar(
                  backgroundImage: AssetImage("lib/Icons/$icon.png"),
                  radius: 30,
                ),
              ...
              ElevatedButton(
                  onPressed: isUserSignedIn()
                      ? () => setState(() {
                            ...
                            comments["username"] = FirebaseAuth
                ...
                if (snapshot.connectionState == ConnectionState.waiting) {
                  return Text(
                    ...
                    style: TextStyle(
                        ...
                        color: const Color.fromARGB(255, 131, 141, 145)),
    ...
    return Column(
      children: [
        ...
        IconButton(
          ...
          tooltip: widget.collapsed ? "Show Books' Bar" : "Hide Books' Bar",
        ),
...
class _GetPostState extends State<GetPost> {
  ...
  void updateLikes(String postId) async {
    ...
    if (!likedPosts.contains(postId)) {
      ...
      await db
          .collection("Users")
          .doc(FirebaseAuth.instance.currentUser!.uid)
          .set({"liked": likedPosts}, SetOptions(merge: true));
      await db.collection("Feeds").doc(postId).set(
          {"likes": postData.data()!["likes"] + 1}, SetOptions(merge: true));
      //likes1++;
    } else {
            ...
            children: [
              Expanded(
                child: Padding(
                  ...
                  child: getBookDetailsColumn(
                      username, book, author, genre, num, commentOn, bookNum),
                ),
              ),
              Expanded(
                flex: 4,
                child: Padding(
                  ...
                  child: Column(
                    ...
                    children: [
                      Row(children: [
                        CircleAvatar(
                          backgroundImage: AssetImage("lib/Icons/$num.png"),
                          radius: 30,
                        ),
                        SizedBox(
                          width: 20,
                                          ...
                                          ? FutureBuilder<IconData>(
                                              ...
                                              builder: (context, snapshot) {
                                                if (snapshot.connectionState ==
                                                  ...
                                                  return Text(
                                                      'Error: ${snapshot.error}'); // Show error icon if the future throws an error
                                                } else {
                                                  IconData iconData =
                                                      snapshot.data!;
          ...
          children: [
            ...
            Expanded(
              child: ListTile(
                  ...
                  subtitle: ReadMoreText(hidden ? "Hidden" : text,
                      textAlign: TextAlign.start,
                      trimLines: 4,
                      trimMode: TrimMode.Line,
                      trimCollapsedText: '...Read more',
  ...
  Widget getBookDetailsColumn(String username, String book, String author,
      String genre, int num, bool commentOn, int bookNum) {
    return Column(
              ...
              Expanded(
                child: ListTile(
                  ...
                  subtitle: Text(
                    ...
                    overflow: TextOverflow.ellipsis,
                    style: TextStyle(
                        fontSize: 20, color: Color.fromARGB(255, 129, 114, 91)),
                    maxLines: 4,
                  ),
...
/*Divider(
  ...
  void initState() {
    ...
    if (isUserSignedIn()) {
      _iconFuture = getFavouriteIcon1(widget.postId);
    }
...
class _HomeScreenState extends State<HomeScreen> {
  ...
  var dbList;
  var selectedSort;
  List sortList = ['Top Rated Books', 'Latest', 'Top Liked Reviews'];
  var collapsed = false;
  var selectedBook = "";
  ...
  Future<List> getDbList(SortType sortBy) async {
    ...
    await db.collection("Feeds").get().then(
      (querySnapshot) {
        for (var docSnapshot in querySnapshot.docs) {
          if (selectedBook == "") {
            docList.add(
                {"postId": docSnapshot!.id!, "postData": docSnapshot!.data()});
    ...
    if (sortBy == SortType.timeDescending) {
      docList.sort(
          (a, b) => b["postData"]["time"].compareTo(a["postData"]["time"]));
    }

    if (sortBy == SortType.timeAscending) {
      docList.sort(
          (a, b) => a["postData"]["time"].compareTo(b["postData"]["time"]));
    }

    if (sortBy == SortType.popularityAscending) {
      docList.sort(
          (a, b) => a["postData"]["likes"].compareTo(b["postData"]["likes"]));
    }

    if (sortBy == SortType.popularityDescending) {
      docList.sort(
          (a, b) => b["postData"]["likes"].compareTo(a["postData"]["likes"]));
    }

    if (sortBy == SortType.topRated) {
      docList.sort(
          (a, b) => b["postData"]["rate"].compareTo(a["postData"]["rate"]));
    }

    if (sortBy == SortType.lessRated) {
      docList.sort(
          (a, b) => a["postData"]["rate"].compareTo(b["postData"]["rate"]));
    }
    ...
    return Scaffold(
          ...
          title: Row(
            ...
            children: [
              ...
              DropdownButton(
                ...
                focusColor: Color.fromARGB(255, 170, 176, 152),
        ...
        drawer: Drawer(
          child: ListView(
            ...
            children: [getDrawerHead(), getDrawerList()],
          ),
        ),
                    ...
                    return ListView.builder(
                      ...
                      itemBuilder: (BuildContext context, int index) {
                        return //Text("hhhhhhhhhh",style: TextStyle(fontSize: 500),);
                            GetPost(
                          username: docList[index]["postData"]['username'],
                          book: docList[index]["postData"]['book']!,
                          text: docList[index]["postData"]['text']!,
                          author: docList[index]["postData"]['author']!,
                          num: docList[index]["postData"]["icon"],
                          rate: docList[index]["postData"]['rate'],
                          likes: docList[index]["postData"]['likes'],
                          genre: docList[index]["postData"]['genre'],
                          postId: docList[index]["postId"],
                          bookNum: docList[index]["postData"]['bookNum'],
                        );
                      },
                    );
                  },
